...
class PascalVOC (object):
    ...
    def __init__(self, base_dir, year='2012'):
        ...
        self.all_image_names = set([os.path.splitext(filename)[0] for filename in os.listdir(self.img_dir)])
    ...
    def __read_all_annotations(self):
        if self.__all_annotations is None or self.__all_image_metadata is None:
            ...
            annotations_df_path = os.path.join(self.ann_dir, 'all_annotations.h5')
            img_meta_df_path = os.path.join(self.ann_dir, 'all_metadata.h5')
            if not os.path.exists(annotations_df_path) or not os.path.exists(img_meta_df_path):
                ...
                for img_i, img_name in enumerate(self.all_image_names):
                    ...
                    for obj in objs:
                        ...
                        anno_data.append([fname, cat, xmin, ymin, xmax, ymax, occluded, truncated, difficult, train,
                                          val, trainval])
                    ...
                    img_meta.append([fname, width, height, depth, train, val, trainval])

                    sys.stdout.write('\r{}'.format(img_i))

                anno_df = pd.DataFrame(
                    anno_data, columns=['image_filename', 'category', 'xmin', 'ymin', 'xmax', 'ymax', 'occluded',
                                        'truncated', 'difficult', 'train', 'val', 'trainval'])

                img_meta_df = pd.DataFrame(
                    img_meta, columns=['image_filename', 'width', 'height', 'depth', 'train', 'val',
                                       'trainval'])
                ...
                self.__all_annotations = pd.read_hdf(annotations_df_path, 'annotations')
                self.__all_image_metadata = pd.read_hdf(img_meta_df_path, 'img_metadata')
    ...
    def get_image_metadata(self, image_names=None, category=None, dataset=None):
        df = self.all_image_metadata
        df = self.__image_names_filter(df, image_names)
        if image_names is not None:
            df = df[df.apply(lambda x: x['image_filename'] in image_names, axis=1)]
        return df
    ...
    def __image_names_filter(self, df, image_names=None):
        if image_names is None:
            ...
            return df[df.apply(lambda x: x['image_filename'] in image_names, axis=1)]
    ...
    def __dataset_filter(self, df, dataset=None):
        if dataset is None:
            ...
            raise ValueError('Invalid dataset \'{}\'; valid values are train, val or trainval'.format(dataset))
